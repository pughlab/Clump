...
make_bam_from_reads <- function(out.dir, out.bam, reader, ...) {
    ...
    out.bam.unsorted <- sub(".bam$", ".unsorted.bam", out.bam)
    writer <- bamWriter(getHeader(reader), out.bam.unsorted)
    reads.list <- list(...)
    for(reads in reads.list) {
        bamSave(writer, reads, refid=0)
    }
    bamClose(writer)

    #sort bam, make bam index
    reader.out <- bamReader(out.bam.unsorted)
    bamSort(reader.out, sub(".bam$", "", out.bam))
    bamClose(reader.out)
    reader.out.sorted <- bamReader(out.bam)
    create.index(reader.out.sorted)
    bamClose(reader.out.sorted)
...
if(dim(as.data.frame(reads.large_insert))[1] > 0) {
	...
	make_bam_from_reads(out.dir, out.bam, reader, reads.large_insert, reads.large_insert.mates, reads.softclipped)
	bamClose(reader)
